...
#
# The mechanism: a CLI like this one reaches its callers as a shim installed inside a single
# application, so a session shipping some OTHER repo's ticket stands in that application's checkout
# while the commit it is reporting lives somewhere else entirely. The In Review comment was built on
# the assumption that the cwd and the commit always agree:
#
#     Merged to main: [`08e63ab`](https://github.com/acme/app/commit/08e63ab) — Kamal deploy in progress
#
# Three false claims in one sentence, on a ticket whose work was an OPEN PR in a different repo: not
# merged, no deploy pipeline in that repo at all, and a link into a repo where the SHA does not exist
# (measured: 404) — a dead link on a ticket's permanent audit trail. The link is the cheap half.
# "Merged to main" is the sentence a human or a closeout session reads to decide whether the work
# shipped, emitted automatically by the tool whose job is recording state — so nobody thinks to
# check it.
#
...
class CliCommitLinkTest < LinearCli::TestCase
  # The coverage needs exactly two distinct slugs and one deploying platform; nothing depends on
  # which. DEPLOYING is where the cwd stands and where a deploy is declared; OTHER is the repo a
  # foreign SHA really lives in, and it deploys nowhere.
  OTHER     = "acme/tools"
  DEPLOYING = "acme/app"
  PLATFORM  = "Kamal"
  ...
  #   landed:  a commit that IS on origin's default branch  (the shipped shape — merged, deploying)
  ...
  def deploying(slug, platform = PLATFORM)
  ...
  test "CONTROL: from a non-deploying checkout, review claims no other-repo link, no deploy, no merge" do
    root, _landed, pushed = checkout(slug: OTHER)

    body = review_body(root, ["--sha", pushed])

    refute_includes body, DEPLOYING, "the link must not point at a repo this commit isn't in"
    refute_includes body, PLATFORM, "nothing declared a deploy for this repo"
    refute_match(/merged/i, body, "the PR was open; review means In Review, not merged")
    assert_includes body, "https://github.com/#{OTHER}/commit/#{pushed}", "link it to the repo it IS in"
    assert_includes body, "Pushed for review", "say what actually happened: pushed, not merged"
  end

  test "CONTROL: standing in the deploying repo with a SHA from another repo emits NO link at all" do
    # The exact invocation behind every one of the five recurrences: the CLI is a shim living inside
    # the deploying app, so cwd is that app while the SHA belongs to another repo. The old code took
    # the cwd's origin regardless and minted a 404; there is nothing here that can be linked
    # correctly, so nothing is linked.
    app, = checkout(slug: DEPLOYING, path: "app")
    _other, _landed, foreign = checkout(slug: OTHER, path: "tools")
    deploying(DEPLOYING) # the cwd IS a deploying repo — so the refutations below aren't vacuous

    body = review_body(app, ["--sha", foreign])

    refute_includes body, "https://github.com/", "an unprovable link is worse than none (AGT-212)"
    refute_includes body, PLATFORM
  ...
  test "--repo owner/name links a commit that lives in another checkout" do
    app, = checkout(slug: DEPLOYING, path: "app")
    _other, _landed, foreign = checkout(slug: OTHER, path: "tools")
    deploying(DEPLOYING) # the cwd deploys, the resolved repo does not — that is the distinction tested

    body = review_body(app, ["--sha", foreign, "--repo", OTHER])

    assert_includes body, "https://github.com/#{OTHER}/commit/#{foreign}"
    refute_includes body, DEPLOYING
    refute_includes body, PLATFORM, "the deploy clause follows the RESOLVED repo, not the cwd"
  end
  ...
  test "with $LINEAR_CLI_DEPLOY_REPOS unset, even a landed commit gets NO deploy clause" do
    ...
    root, landed, = checkout(slug: DEPLOYING)
  ...
  test "PAIRED CONTROL: in a configured deploying checkout, a landed SHA still links there and still deploys" do
    ...
    assert_includes body, "https://github.com/#{DEPLOYING}/commit/#{landed}", "the linking path is unchanged"
    assert_includes body, "#{PLATFORM} deploy in progress", "the caller declared this repo deploys"
    assert_includes body, "Merged to main", "and it really is on origin/main, so the claim is earned"
    assert_includes body, "landed on main (#{DEPLOYING})", "the commit subject rides along as before"
  end
  ...
  test "in a deploying repo, a commit that is NOT on origin/main gets neither the merge claim nor the deploy clause" do
    ...
    root, _landed, pushed = checkout(slug: DEPLOYING)
    deploying(DEPLOYING)

    body = review_body(root, ["--sha", pushed])

    assert_includes body, "https://github.com/#{DEPLOYING}/commit/#{pushed}", "the repo is still known"
    refute_includes body, PLATFORM, "nothing has been deployed — it isn't on main"
    refute_match(/merged/i, body)
  end
  ...
  test "--merged asserts the merge (and the deploy) when git cannot see it, --not-merged withdraws it" do
    ...
    root, landed, pushed = checkout(slug: DEPLOYING)
    deploying(DEPLOYING)

    assert_includes review_body(root, ["--sha", pushed, "--merged"]), "Merged to main"
    assert_includes review_body(root, ["--sha", pushed, "--merged"]), "#{PLATFORM} deploy in progress"
    refute_match(/merged/i, review_body(root, ["--sha", landed, "--not-merged"]))
    refute_includes review_body(root, ["--sha", landed, "--not-merged"]), PLATFORM
  end

  test "--no-deploy silences the clause for a deploying repo, --deploy adds it for any repo" do
    app, landed, = checkout(slug: DEPLOYING, path: "app")
    other, other_landed, = checkout(slug: OTHER, path: "tools")
    deploying(DEPLOYING)

    refute_includes review_body(app, ["--sha", landed, "--no-deploy"]), "deploy in progress"
    assert_includes review_body(other, ["--sha", other_landed, "--deploy"]), "deploy in progress",
  ...
  test "$LINEAR_CLI_DEPLOY_REPOS is the only source of the allowlist, and an empty value disables it" do
    ...
    ENV["LINEAR_CLI_DEPLOY_REPOS"] = "#{OTHER}=#{PLATFORM}"
    assert_includes review_body(other, ["--sha", other_landed]), "#{PLATFORM} deploy in progress"
    refute_includes review_body(app, ["--sha", landed]), "deploy in progress",
                    "the list is exhaustive — a repo absent from it does not deploy"

    ENV["LINEAR_CLI_DEPLOY_REPOS"] = ""
    refute_includes review_body(other, ["--sha", other_landed]), "deploy in progress",
  ...
  test "commit links to this checkout's repo, and omits the link for a SHA that is not here" do
    app, landed, = checkout(slug: DEPLOYING, path: "app")
    _other, _other_landed, foreign = checkout(slug: OTHER, path: "tools")

    own = commit_body(app, ["--sha", landed])
    assert_includes own, "https://github.com/#{DEPLOYING}/commit/#{landed}"
    assert_includes own, "landed on main (#{DEPLOYING})"

    foreign_body = commit_body(app, ["--sha", foreign])
    refute_includes foreign_body, "https://github.com/", "the same 404 the review verb used to mint"
    assert_includes foreign_body, foreign[0, 8]

    named = commit_body(app, ["--sha", foreign, "--repo", OTHER])
    assert_includes named, "https://github.com/#{OTHER}/commit/#{foreign}"
  end
